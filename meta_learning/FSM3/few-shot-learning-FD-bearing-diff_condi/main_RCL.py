...
from Dataload import SimpleDataLoader, MetaDataLoader   # Dataload.py adds random convolution layer
...
if __name__ == "__main__":
    ...
    if args.data_type != 'origi':
        train_data_path = os.path.join(args.base_data_path, args.source_dataset + '.h5')
        test_data_path = os.path.join(args.base_data_path, args.target_dataset + '.h5')
    ...
    if args.method == 'matching_network':
        ...
        for epo in range(args.meta_epochs):
            ...
            #model.train_metric_iter(data_loader_train, epo,randomconv=True)
            model.train_metric_iter(data_loader_train, epo,randomconv=False)
