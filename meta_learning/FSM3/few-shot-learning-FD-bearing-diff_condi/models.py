...
class MatchingNetwork(nn.Module):
    ...
    def forward_fce(self, x):
        ...
        # get support label in one-hot form
        y_support = torch.LongTensor(np.repeat(range(n_way), self.args.n_shot))
    ...
    def train_iter(self, data_loader, epoch):
        ...
        for i, (x, _) in enumerate(data_loader):
            ...
            y_query = torch.LongTensor(np.repeat(range(n_way), self.args.n_query))  # y_query: n_way*n_query vector
...
class MatchingNetwork_feat_space(nn.Module):
    ...
    def train_metric_iter(self, data_loader, epoch,randomconv=True):
        self.backbone.train(False)
        loss_list, accu_list = [], []
        ...
        for i, (x, _) in enumerate(data_loader):
            ...
            if randomconv==True:
                ...
                if p > 0.5:
                    ...
                    x = Conv(x.reshape(-1, C, W)).reshape(N,S, C,  W)
