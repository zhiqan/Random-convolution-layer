...
class Data_CWRU:
    ...
    def get_data(self, train_mode=True, n_each_class=5, sample_len=1024, normalize=True):
...
class CNN_DataGenerator_torch(data.Dataset):
    def __init__(self, mode, ways, shot):
        ...
        self.sample_len = 1024
        self.n_way = ways
        self.task_mode = True if ways == 3 else False
        self.__getdata__(mode)
        self.ID_list = np.arange(0, len(self.x))
...
if __name__ == "__main__":
    ...
    train_dataset = l2l.data.MetaDataset(MAML_Dataset(mode='train', ways=3))
