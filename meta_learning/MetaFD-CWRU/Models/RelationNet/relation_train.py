...
vis = visdom.Visdom(env='yancy_meta2')
...
class RelationNet_learner(object):
    ...
    def fast_adapt(self, data, labels, loss_fun, query_num, shots, ways):
        # Separate data into adaptation/evaluation sets
        sort = torch.sort(labels)
        data = data.squeeze(0)[sort.indices].squeeze(0)
        labels = labels.squeeze(0)[sort.indices].squeeze(0)
    ...
    def train(self, save_path, shots):
        ...
        for ep in range(Epochs):
            ...
            for epi in range(Episodes):
                ...
                #data=self.fast_adapt2(data,labels,loss_fun,query_num, shots, train_ways)
 
                loss, acc = self.fast_adapt(data, labels, loss_fun, query_num, shots, train_ways)
            ...
            if (ep+1) >=20 and (ep+1)%2==0:  # generally (ep+1) >=200
                if input('\n== Stop training? == (y/n)\n').lower() == 'y':
    ...
    def test(self, load_path, shots):
        ...
        for i, batch in enumerate(test_tasks):
            with torch.no_grad():
                data, labels = batch
                data, labels = data.to(device), labels.to(device)
                loss, acc = self.fast_adapt(data,labels, loss_fun, query_num, shots, test_ways)
                meta_valid_error += loss.item()
                meta_valid_accuracy += acc.item()
...
if __name__ == "__main__":
    ...
    Net = RelationNet_learner(ways=3)  # T2
    ...
    if input('Train? y/n\n').lower() == 'y':
        ...
        path = r"D:\\论文+无监督下基于小样本数据的旋转机械故障诊断研究\\故障诊断例子\\MetaFD-main\\Models\\ProtoNet\\5shot_RLNET_padT1-T0"
        Net.train(save_path=path, shots=5)
    ...
    if input('Test? y/n\n').lower() == 'y':
        ...
        load_path = r"D:\\论文+无监督下基于小样本数据的旋转机械故障诊断研究\\故障诊断例子\\MetaFD-main\\Models\\ProtoNet\\5shot_RLNET_padT1-T0_ep20(1)"
        Net.test(load_path, shots=5)
