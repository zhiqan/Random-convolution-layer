...
vis = visdom.Visdom(env='7_12_AIML_T0-T1')
...
class MAML_learner(object):
    ...
    def test(self, load_path, inner_steps=10, shots=5):
        ...
        for _ in range(meta_batch_size):
            ...
            #print(meta_test_error / meta_batch_size)
            print(meta_test_accuracy / meta_batch_size)
...
Net = MAML_learner(ways=6) #HIT_dataset
...
if input('Train? y/n\n').lower() == 'y':
    ...
    path = r"D:\\论文+无监督下基于小样本数据的旋转机械故障诊断研究\\故障诊断例子\\MetaFD-main\\Models\\MAML\\5shot_MAML_7_12_PADT0-T2"
    Net.train(save_path=path, shots=5)
...
if input('Test? y/n\n').lower() == 'y':
    ...
    load_path = r"D:\论文+无监督下基于小样本数据的旋转机械故障诊断研究\故障诊断例子\MetaFD-main\Models\MAML\5shot_MAML_7_12_PADT0-T2_ep200"
    Net.test(load_path, inner_steps=10, shots=5)
...
load_path = r"D:\论文+无监督下基于小样本数据的旋转机械故障诊断研究\故障诊断例子\MetaFD-main\Models\MAML\5shot_MAML_7_12_PADT0-T1_ep200"
Net.test(load_path, inner_steps=10, shots=5)
