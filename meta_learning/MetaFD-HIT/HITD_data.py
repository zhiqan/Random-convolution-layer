...
from cwru_path import T0, T1
from mat2csv import get_data_csv
from init_utils import one_hot_encode, sample_label_shuffle
from init_utils import my_normalization as normalization

N_TRAIN_EACH_CLASS = 200
...
class Data_CWRU:
    ...
    def get_data(self, train_mode=True, n_each_class=6, sample_len=1024, normalize=True):
        ...
        for i in range(n_way):
            data = get_data_csv(file_dir=data_file[i], num=data_size, header=0, shift_step=512)  # (N, len)
...
class MANN_DataGenerator(Sequence):
    ...
    def __getdata__(self, mode):
        if mode=='train':
            ...
            data = Data_CWRU().get_data(train_mode=False, n_each_class=500,
                                        sample_len=self.sample_len, normalize=True)
            if mode == 'validation':
                self.x, self.y = data[0][:, :250], data[1][:, :250]
            elif mode == 'test':
                self.x, self.y = data[0], data[1]
            else:
                exit('Mode error')
...
class CNN_DataGenerator_torch(data.Dataset):
    def __init__(self, mode, ways, shot):
        ...
        self.sample_len = 1024
        self.n_way = 6
        self.task_mode = True if ways == 6 else False
        self.__getdata__(mode)
        self.ID_list = np.arange(0, len(self.x))
...
if __name__ == "__main__":
    ...
    train_dataset = l2l.data.MetaDataset(MAML_Dataset(mode='train', ways=6))
