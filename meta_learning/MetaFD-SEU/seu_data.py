...
class Data_CWRU:
    ...
    def get_data(self, train_mode=True, n_each_class=50, sample_len=1024, normalize=True):
...
class CNN_DataGenerator_torch(data.Dataset):
    def __init__(self, mode, ways, shot):
        ...
        self.sample_len = 1024
        self.n_way = ways
        self.task_mode = True if ways == 4 else False
        self.__getdata__(mode)
        self.ID_list = np.arange(0, len(self.x))
...
class MAML_Dataset(data.Dataset):
    def __init__(self, mode, ways):
        super().__init__()
        self.sample_len = 1024
        self.task_mode = True if ways == 5 else False
        self.__getdata__(mode)
    ...
    def __getdata__(self, mode):
        if mode == 'train':
            ...
            data = Data_CWRU(self.task_mode).get_data(train_mode=False, n_each_class=500,
                                                      sample_len=self.sample_len, normalize=True)
            if mode == 'validation':
                self.x, self.y = data[0][:, :250], data[1][:, :250]
            elif mode == 'test':
